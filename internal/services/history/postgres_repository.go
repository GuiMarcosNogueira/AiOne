...
func (r *PostgresRepository) Insert(ctx context.Context, params InsertParams) (Entry, error) {
	...
	query := `INSERT INTO user_context_history
		(user_id, session_id, provider_name, role, message, media_type, media_path, tokens_estimated)
		VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
		RETURNING id, user_id, session_id, provider_name, role, message, media_type, media_path, tokens_estimated, created_at`
	var sessionID any
	if params.SessionID != "" {
...
func (r *PostgresRepository) List(ctx context.Context, userID, provider string) ([]Entry, error) {
	...
	query := `SELECT id, user_id, session_id, provider_name, role, message, media_type, media_path, tokens_estimated, created_at
...
// ListBySession returns entries tied to a specific session.
func (r *PostgresRepository) ListBySession(ctx context.Context, userID, sessionID string) ([]Entry, error) {
	if err := r.ensure(); err != nil {
		return nil, err
	}
	query := `SELECT id, user_id, session_id, provider_name, role, message, media_type, media_path, tokens_estimated, created_at
		...
		ORDER BY created_at ASC`
	rows, err := r.db.QueryContext(ctx, query, userID, sessionID)
	if err != nil {
...
// DeleteSession removes every entry associated with a specific chat session.
func (r *PostgresRepository) DeleteSession(ctx context.Context, userID, sessionID string) error {
	if err := r.ensure(); err != nil {
		return err
	}
	_, err := r.db.ExecContext(ctx, `DELETE FROM user_context_history WHERE user_id=$1 AND session_id=$2`, userID, sessionID)
	return err
}
...
func scanEntry(row rowScanner) (Entry, error) {
	...
	if err := row.Scan(&e.ID, &e.UserID, &sessionID, &e.ProviderName, &e.Role, &e.Message, &e.MediaType, &e.MediaPath, &e.TokensEstimated, &e.CreatedAt); err != nil {
